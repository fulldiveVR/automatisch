...
const parametrize = function (obj: any) {
  let arrayOfArrays, sortedParams: string, symbol: string;
  arrayOfArrays = _.toPairs(obj).sort();
  symbol = '';
  sortedParams = '';
...
export const okSignature = (postData: any, accessToken: string) => {
  var secret, sortedParams;
  sortedParams = parametrize(postData);
  secret = md5(accessToken + postData.application_key);
